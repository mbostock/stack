function stack() {
  ...
  if (touchy) {
    ...
    var background = d3.select("body").append("div", "section")
    ...
    var sectionAndBackground = d3.selectAll(nodes.concat(background.node()))
    ...
    var sectionPrevious = d3.select(null),
        sectionCurrent = d3.select(nodes[0]),
        sectionNext = d3.select(nodes[1]);
  ...
  function dispatchEvent(event, i) {
    var target = nodes[i], sourceEvent = event.sourceEvent = d3.event;
    try {
      d3.event = event;
  ...
  function reposition() {
    ...
    if (i !== i1) {
      if (i1 === i + 1) { // advance one
        ...
        sectionNext = d3.select(nodes[i1 + 1]).interrupt().style("display", "block").style("opacity", 0).style("z-index", 0);
        ...
        sectionCurrent.transition().style("opacity", 1);
        sectionPrevious = d3.select(nodes[i1 - 1]).interrupt().style("display", "block").style("opacity", 0).style("z-index", 0);
      } else { // skip
        sectionPrevious.interrupt().style("display", "none").style("opacity", 0).style("z-index", 0).each(deactivate);
        sectionCurrent.interrupt().style("display", "none").style("opacity", 0).style("z-index", 0).each(deactivate);
        sectionNext.interrupt().style("display", "none").style("opacity", 0).style("z-index", 0).each(deactivate);
        sectionPrevious = d3.select(nodes[i1 - 1]).interrupt().style("display", "block").style("opacity", 0).style("z-index", 0).each(deactivate);
        sectionCurrent = d3.select(nodes[i1]).interrupt().style("display", "block").style("opacity", 1).style("z-index", 2).each(activate);
        sectionNext = d3.select(nodes[i1 + 1]).interrupt().style("display", "block").style("opacity", 0).style("z-index", 0).each(deactivate);
      }
      i = i1;
    }
  ...
  stack.on = function() {
    return arguments.length ? (dispatch.on.apply(dispatch, arguments), stack) : dispatch.on.apply(dispatch, arguments);
  };
